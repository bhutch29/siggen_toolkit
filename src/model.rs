...
impl HttpClientModel {
    fn create_request(&self, stem: &str) -> reqwest::blocking::RequestBuilder {
...
impl Model for HttpClientModel {
    ...
    fn file_exists(&self, path: &Path) -> bool {
        ...
        let response = self
            .create_request(&format!("file-exists{}", path.to_string_lossy()))
            .send();
        match response {
    ...
    fn logging_valid_paths(&self) -> Vec<PathBuf> {
        ...
        match self.create_request("logging/valid-paths").send() {
            Ok(response) => serde_json::from_str(&response.text().unwrap_or_default())
                .ok()
                .unwrap_or_default(),
            Err(err) => {
    ...
    fn logging_get_config_from(&self, path: &Path) -> Option<LoggingConfiguration> {
        ...
        let response = self
            .create_request(&format!("logging/config{}", path.to_string_lossy()))
            .send();
        match response
        {
            Ok(response) => serde_json::from_str(&response.text().unwrap_or_default())
